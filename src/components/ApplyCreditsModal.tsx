...
import { PaymentMade, InwardInvoice } from '@/types';
import { formatDate, formatCurrency } from '@/utils';
import Modal from '@/components/Modal';
import Button from '@/components/Button';

interface Props {
  payment: PaymentMade; // the advance payment whose credits to apply
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

const ApplyCreditsModal: React.FC<Props> = ({ payment, isOpen, onClose, onSuccess }) => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const res = await axios.get('/api/v1/getall-inward', {
          params: { vendorId: payment.vendorId, limit: 100 },
          headers: { Authorization: `Bearer ${token}` },
        });
        if (res.data?.success) {
          const bills: InwardInvoice[] = res.data.data || [];
          const outstanding = bills
            .map((inv) => ({
              ...
              balanceDue: inv.totalCost - (inv.amountPaid || 0),
            }))
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        '/api/v1/paymentsmade/apply-credits',
        { vendorId: payment.vendorId, allocations: allocs, date: new Date().toISOString() },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get(`/api/v1/paymentsmade/${createdPaymentId}/pdf`, {
        responseType: 'blob',
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Apply Credits — ${payment.paymentNumber}`}
      size="lg"
    >
          ...
          <div className="overflow-x-auto rounded border border-gray-200">
            <table className="min-w-full divide-y divide-gray-200 text-[13px]">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-2.5 text-left font-semibold text-gray-500">Bill No</th>
              ...
              <tbody className="divide-y divide-gray-100 bg-white">
                {invoices.map((inv) => {
                  ...
                  return (
                    <tr key={inv.id} className={val > 0 ? 'bg-teal-50/20' : ''}>
                      ...
                      <td className="px-4 py-2.5 text-right">
                        <div className="relative inline-block w-full">
                          ...
                          <input
                            ...
                            className="w-full pl-6 pr-2 py-1 text-right border border-gray-300 rounded outline-none focus:ring-1 focus:ring-teal-500 font-semibold text-teal-700 bg-white text-[13px]"
                            placeholder="0.00"
                          />
                        </div>
                      </td>
              ...
              {totalAllocated > 0 && (
                <tfoot className="bg-gray-50">
                  <tr>
                    ...
                    <td className="px-4 py-2 text-right font-bold text-teal-700 text-[13px]">
        ...
        <div className="flex items-center justify-between pt-2 border-t gap-3">
          <div className="flex gap-2">
            <Button
              ...
              className="bg-teal-600 hover:bg-teal-700 text-white px-5 py-2 text-xs font-bold rounded disabled:opacity-50"
            >
              {submitting ? (
