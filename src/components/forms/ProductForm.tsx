...
import { X, Upload, QrCode, Barcode, Package, AlertTriangle } from 'lucide-react';
...
const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const units = [
    'PCS', 'KG', 'GRAM', 'LITER', 'ML', 'METER', 'CM', 'BOX', 'CARTON', 'DOZEN'
  ];

  const storageConditions = [
    'Room Temperature', 'Cool & Dry', 'Refrigerated (2-8°C)', 
    'Frozen (-18°C)', 'Controlled Temperature', 'Hazardous Storage'
  ];
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 2) {
      ...
      if (formData.reorderLevel && formData.minStockLevel && 
          formData.reorderLevel >= formData.minStockLevel) {
        newErrors.reorderLevel = 'Reorder level must be less than min stock level';
      }
    }
    ...
    if (step === 3 && formData.isPerishable) {
      ...
      if (!formData.storageCondition?.trim()) {
        newErrors.storageCondition = 'Storage condition is required for perishable items';
      }
    }
    ...
    <div className="space-y-4">
      ...
      <div className="mt-6 p-4 bg-gray-50 rounded-lg">
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          <div><span className="font-medium">Code:</span> {formData.code}</div>
          <div><span className="font-medium">Name:</span> {formData.name}</div>
          <div><span className="font-medium">Category:</span> {categories.find(c => c.id === formData.categoryId)?.name}</div>
          <div><span className="font-medium">Unit:</span> {formData.unit}</div>
          <div><span className="font-medium">Min Stock:</span> {formData.minStockLevel}</div>
          <div><span className="font-medium">Max Stock:</span> {formData.maxStockLevel}</div>
          <div><span className="font-medium">Perishable:</span> {formData.isPerishable ? 'Yes' : 'No'}</div>
          <div><span className="font-medium">Active:</span> {formData.isActive ? 'Yes' : 'No'}</div>
        </div>
      </div>
    </div>
  );
